...
Figure~\ref{fig:motivation} summarizes the study motivation: regionally heterogeneous IDR labels, benchmark interpretation risks and the resulting need for sequence-only inputs, region-aware specialization, calibration and separated evidence levels.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figure_project_motivation.pdf}
\caption{Project motivation and claim boundary. (a) Intrinsic disorder labels are regionally heterogeneous, spanning terminal and internal IDRs, short and long disorder patterns, ordered residues and unknown labels that must be masked during evaluation. (b) Benchmark interpretation requires explicit handling of prevalence differences, homologous training examples and raw-score calibration. Disorder fractions are from the three independent test sets, and NR25 removed protein counts are from target-specific MMseqs2 filtering. (c) The project therefore keeps the primary model sequence-only, adds region-aware adapter specialization and evaluates full-benchmark performance, NR25 robustness, hard-case strata and Platt-calibrated uncertainty as separate evidence levels.}
\label{fig:motivation}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figure1_method_overview.pdf}
\caption{Method architecture. A protein token track is expanded into four residue-level architecture layers. (a) Sequence-only residue representation combines frozen ESM2-t33 residue embeddings, amino-acid one-hot features and relative-position features. (b) A P4.6 \backbone{} checkpoint initializes the input projection and multi-kernel dilated TCN blocks, which are frozen during P4.8 specialization. (c) Low-rank SDR, LDR, terminal-IDR and internal-IDR adapters generate region-specific expert logits that are mixed by a residue-level MoE gate. (d) Three-seed score averaging and validation-fitted Platt calibration produce residue-level disorder probabilities, binary calls and entropy-based uncertainty.}
\label{fig:method}
\end{figure*}
...
\input{tables/table2_fusionencoder_style_main.tex}
